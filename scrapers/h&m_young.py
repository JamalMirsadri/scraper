...
BASE_URL = "https://www2.hm.com/pt_pt/kids/member-prices/kids-9-14y.html"
SOURCE_STORE = "H&M"
OUTPUT_FILE = "h&m_young_products.csv"
PROGRESS_FILE = f"h&m_young_progress{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def parse_hit(hit):
    """
    استخراج قیمت‌ها از لیست 'prices' هر محصول.

    راه‌حل نهایی و مطمئن: به‌جای تکیه بر نام دقیق priceType (که در
    نسخه‌های مختلف API با نام‌های متفاوتی مثل whitePrice/redPrice یا
    mePrice/membershipPrice و... می‌آید و پیش‌بینی همهٔ حالت‌ها سخت
    است)، همهٔ مقادیر عددی price موجود در آرایهٔ prices جمع‌آوری
    می‌شوند:
      - OriginalPrice = بیشترین قیمت موجود در لیست (قیمت غیرعضو/اصلی)
      - OutletPrice   = کمترین قیمت موجود در لیست (قیمت تخفیف یا
        قیمت اعضا، هرکدام که کمتر باشد)
    اگر فقط یک قیمت وجود داشت، هر دو ستون برابر همان مقدار می‌شوند.
    این روش کاملاً مستقل از تغییر نام‌گذاری priceType در API کار
    می‌کند.
    """
    global DEBUG_PRICES_DONE

    title = hit.get("title", "")
    prices = hit.get("prices", [])

    if not DEBUG_PRICES_DONE and prices:
        print("نمونهٔ خام آرایهٔ prices برای اولین محصول:")
        print(json.dumps(prices, ensure_ascii=False, indent=2))
        DEBUG_PRICES_DONE = True
    ...
    for p in prices:
        ...
        if val is None:
            continue
        try:
            numeric_prices.append(float(str(val).replace(",", ".")))
        except (ValueError, TypeError):
            continue

    if numeric_prices:
        original_price = str(max(numeric_prices))
        outlet_price = str(min(numeric_prices))
    else:
        original_price = ""
        outlet_price = ""
...
def main():
    ...
    if failed_pages:
        ...
        print("برای تلاش مجدد فقط همین صفحات، دوباره اسکریپت را اجرا کنید یا فایل progress را دستی ویرایش کنید.")
